...
class LabEventDate extends React.Component {
  ...
  async updateMapping(updateArray) {
    ...
    const dataStoreNameSpace = await getDataStoreNameSpace(this.props.orgUnitId)
    .then((response) => {      
    ...
    if (typeof dataStoreNameSpace === 'undefined') {
      await createDateStoreNameSpace('api/dataStore/whonet/'+this.props.orgUnitId, JSON.stringify(this.props.orgUnitId)).then(info=>{
          console.log("Info: ", info.data);
      });
      await metaDataUpdate('api/dataStore/whonet/'+this.props.orgUnitId, JSON.stringify({"elements":[], "attributes":[], "options": [], "eventDate": updatedEventDate }) )
      .then((response) => {
      ...
      await metaDataUpdate('api/dataStore/whonet/'+this.props.orgUnitId, JSON.stringify(finalPayload) )
      .then((response) => {
        if(response.data.httpStatus === "OK" ){
          this.setState({
            loading: false,
